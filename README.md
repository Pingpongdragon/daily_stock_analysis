...
[**项目简介**](#-项目简介) · [**功能特性**](#-功能特性) · [**快速开始**](#-快速开始) · [**推送效果**](#-推送效果) · [**完整指南**](docs/full-guide.md) · [**常见问题**](docs/FAQ.md) · [**更新日志**](docs/CHANGELOG.md)
...
`daily_stock_analysis` 是一套**基于 AI 大模型的自动化股票分析系统**，适用于个人投资者对 A 股、港股、美股自选股进行每日跟踪与决策辅助。

**它解决什么问题？**

每天收盘后，手动逐一查看技术指标、搜集相关新闻、判断买卖时机既耗时又费力。本项目将这一流程完全自动化：

1. **拉取行情数据** — 从 AkShare / Tushare / YFinance 等多数据源获取 K 线、均线、筹码分布、实时行情
2. **搜集舆情新闻** — 通过 Tavily / Bocha / SerpAPI 等搜索接口抓取最新个股相关新闻
3. **AI 综合分析** — 调用 Gemini / Claude / DeepSeek / GPT 等大模型，按照内置交易纪律（严禁追高、趋势交易、精确点位）生成「决策仪表盘」
4. **多渠道推送** — 将分析报告自动发送至企业微信、飞书、Telegram、钉钉、邮件、Discord 等

整个流程**每天只需一次触发**（可通过 GitHub Actions 定时执行，无需自备服务器），即可在收到推送后快速判断当日操作方向。

> 本系统仅提供技术分析与信息聚合，不构成任何投资建议。最终操作决策须由用户自行判断并承担相应风险。
